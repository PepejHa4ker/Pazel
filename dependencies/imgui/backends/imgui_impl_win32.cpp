...
//  [X] Platform: Keyboard support. Since 1.87 we are using the io.AddKeyEvent() functions. Pass ImGuiKey values to all key functions e.g. ImGui::IsKeyPressed(ImGuiKey_Space). [Legacy VK_* values will also be supported unless IMGUI_DISABLE_OBSOLETE_KEYIO is set]
...
// Call from your application's message handler. Keep calling your message handler unless this functions returns TRUE.
...
#if 0
// Copy this line into your .cpp file to forward declare the functions.
...
// Helper functions to enable DPI awareness without setting up a manifest
...
// [experimental]
// Borrowed from GLFW's functions updateFramebufferTransparency() in src/win32_window.c
