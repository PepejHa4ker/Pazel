...
// Using dear imgui via a shared library is not recommended, because we don't guarantee backward nor forward ABI compatibility (also functions call overhead, as dear imgui is a call-heavy API)
#ifndef IMGUI_API
#define IMGUI_API
#endif
...
// Disable some of MSVC most aggressive Debug runtime checks in functions header/footer (used in some simple/low-level functions)
...
struct ImGuiPlatformImeData;        // Platform IME data for io.SetPlatformImeDataFn() functions.
...
typedef int     (*ImGuiInputTextCallback)(ImGuiInputTextCallbackData* data);    // Callback functions for ImGui::InputText()
typedef void    (*ImGuiSizeCallback)(ImGuiSizeCallbackData* data);              // Callback functions for ImGui::SetNextWindowSizeConstraints()
...
namespace ImGui
{
    ...
    //    BeginPopup/EndPopup, etc. where the EndXXX call should only be called if the corresponding BeginXXX functions
    ...
    // Style read access
    // - Use the style editor (ShowStyleEditor() functions) to interactively see what the colors are)
    ...
    // - For all the Float2/Float3/Float4/Int2/Int3/Int4 versions of every functions, note that a 'float v[X]' functions argument is the same as 'float* v',
    ...
    // - Legacy: Pre-1.78 there are DragXXX() functions signatures that takes a final `float power=1.0f' argument instead of the `ImGuiSliderFlags flags=0' argument.
    ...
    // - Adjust format string to decorate the value with a prefix, a suffix, or adapt the editing and display precision e.g. "%.3f" -> 1.234; "%5.2f secs" -> 01.23 secs; "Biscuit: %.0f" -> Biscuit: 1; etc.
    // - Format string may also be set to NULL or use the default format ("%f" or "%d").
    // - Legacy: Pre-1.78 there are SliderXXX() functions signatures that takes a final `float power=1.0f' argument instead of the `ImGuiSliderFlags flags=0' argument.
    //   If you get a warning converting a float to ImGuiSliderFlags, read https://github.com/ocornut/imgui/issues/3361
    ...
    // - Note that in C++ a 'float v[X]' functions argument is the _same_ as 'float* v', the array syntax is just a way to document the number of elements that are expected to be accessible.
    ...
    IMGUI_API bool          IsItemClicked(ImGuiMouseButton mouse_button = 0);                   // is the last item hovered and mouse clicked on? (**)  == IsMouseClicked(mouse_button) && IsItemHovered()Important. (**) this it NOT equivalent to the behavior of e.g. Button(). Read comments in functions definition.
    ...
    // Clipboard Utilities
    // - Also see the LogToClipboard() functions to capture GUI into clipboard, or easily output text data to the clipboard.
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_EnterReturnsTrue    = 1 << 5,   // Return 'true' when Enter is pressed (as opposed to every time the value was modified). Consider looking at the IsItemDeactivatedAfterEdit() functions.
...
struct ImGuiIO
{
    ...
    // Other state maintained from data above + IO functions calls
...
// The callback functions should return 0 by default.
...
struct ImGuiInputTextCallbackData
{
    ...
    // - To modify the text buffer in a callback, prefer using the InsertChars() / DeleteChars() functions. InsertChars() will take care of calling the resize callback if necessary.
    ...
    // Use those functions to benefit from the CallbackResize behaviors. Calling those functions reset the selection.
...
struct ImGuiTableColumnSortSpecs
{
    ...
    ImGuiSortDirection          SortDirection : 8;  // ImGuiSortDirection_Ascending or ImGuiSortDirection_Descending (you can use this or SortSign, whichever is more convenient for your sort functions)
...
// The expected behavior from your rendering functions is 'if (cmd.UserCallback != NULL) { cmd.UserCallback(parent_list, cmd); } else { RenderTriangles() }'
...
// The renderer backend needs to handle this special value, otherwise it will crash trying to call a functions at this address.
...
struct ImDrawCmd
{
    ...
    ImDrawCallback  UserCallback;       // 4-8  // If != NULL, call the functions instead of rendering the vertices. clip_rect and texture_id will be set normally.
    void*           UserCallbackData;   // 4-8  // The draw callback code can access this.

    ImDrawCmd() { memset(this, 0, sizeof(*this)); } // Also ensure our padding fields are zeroed

    // Since 1.83: returns ImTextureID associated with this draw call. Warning: DO NOT assume this is always same as 'TextureId' (we will change this functions for an upcoming feature)
...
// all command lists are passed to your ImGuiIO::RenderDrawListFn functions for rendering.
...
struct ImDrawList
{
    ...
    IMGUI_API void  PushClipRect(const ImVec2& clip_rect_min, const ImVec2& clip_rect_max, bool intersect_with_current_clip_rect = false);  // Render-level scissoring. This is passed down to your render functions but not used for CPU-side coarse clipping. Prefer using higher-level ImGui::PushClipRect() to affect logic (hit-testing and widget culling)
    ...
    // Advanced
    IMGUI_API void  AddCallback(ImDrawCallback callback, void* callback_data);  // Your rendering functions must check for 'UserCallback' in ImDrawCmd and call the functions instead of rendering triangles.
...
// (Optional) Support for IME (Input Method Editor) via the io.SetPlatformImeDataFn() functions.
