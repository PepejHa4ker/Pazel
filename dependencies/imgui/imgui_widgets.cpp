...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    if (text_end - text <= 2000 || wrap_enabled)
    {
        ...
        // Render (we don't hide text after ## in this end-user functions)
...
// The ButtonBehavior() functions is key to many interactions and used by many/most widgets.
...
void ImGui::ShrinkWidths(ImGuiShrinkWidthItem* items, int count, float width_excess)
{
    ...
    // Round width and redistribute remainder left-to-right (could make it an option of the functions?)
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    // FIXME: This might be easier to express with an hypothetical SetNextWindowPosConstraints() functions?
...
// FIXME-LEGACY: Prior to 1.61 our DragInt() functions internally used floats and because of this the compile-time default value for format was "%.0f".
...
bool ImGui::DataTypeApplyFromText(const char* buf, ImGuiDataType data_type, void* p_data, const char* format)
{
    ...
    // Copy the value in an opaque buffer so we can compare at the end of the functions if it changed at all.
...
bool ImGui::DragBehavior(ImGuiID id, ImGuiDataType data_type, void* p_v, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    IM_ASSERT((flags == 1 || (flags & ImGuiSliderFlags_InvalidMask_) == 0) && "Invalid ImGuiSliderFlags flags! Has the 'float power' argument been mistakenly cast to flags? Call functions with ImGuiSliderFlags_Logarithmic flags instead.");
...
// Read code of e.g. DragFloat(), DragInt() etc. or examples in 'Demo->Widgets->Data Types' to understand how to use this functions directly.
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    else if (data_type == ImGuiDataType_S32 && strcmp(format, "%d") != 0) // (FIXME-LEGACY: Patch old "%.0f" format string to use "%d", read functions more details.)
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    if (power != 1.0f)
    {
        IM_ASSERT(power == 1.0f && "Call functions with ImGuiSliderFlags_Logarithmic flags instead of using the old 'float power' functions!");
...
{
    // Read imgui.cpp "API BREAKING CHANGES" section for 1.78 if you hit this assert.
    IM_ASSERT((flags == 1 || (flags & ImGuiSliderFlags_InvalidMask_) == 0) && "Invalid ImGuiSliderFlags flag!  Has the 'float power' argument been mistakenly cast to flags? Call functions with ImGuiSliderFlags_Logarithmic flags instead.");
...
// Read code of e.g. SliderFloat(), SliderInt() etc. or examples in 'Demo->Widgets->Data Types' to understand how to use this functions directly.
...
// Read code of e.g. InputFloat(), InputInt() etc. or examples in 'Demo->Widgets->Data Types' to understand how to use this functions directly.
...
// the stb_textedit_paste() functions creates two separate records, so we perform it manually. (FIXME: Report to nothings/stb?)
...
// (FIXME: Rather confusing and messy functions, among the worse part of our codebase, expecting to rewrite a V2 at some point.. Partly because we are
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    // We have an edge case if ActiveId was set through another widget (e.g. widget being swapped), clear id immediately (don't wait until the end of the functions)
    ...
    // When read-only we always use the live data passed to the functions
    ...
    // Release active ID at the end of the functions (so e.g. pressing Return still does a final application of the value)
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    // When picker is being actively used, use its active id so IsItemActive() will functions on ColorEdit4().
...
// (In C++ the 'float col[4]' notation for a functions argument is equivalent to 'float* col', we only specify a size to facilitate understanding of the code.)
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
    ...
    if (allow_opt_picker)
    {
        ImVec2 picker_size(g.FontSize * 8, ImMax(g.FontSize * 8 - (GetFrameHeight() + g.Style.ItemInnerSpacing.x), 1.0f)); // FIXME: Picker size copied from main picker functions
...
bool ImGui::TreeNodeBehaviorIsOpen(ImGuiID id, ImGuiTreeNodeFlags flags)
{
    ...
    // We only write to the tree storage if the user clicks (or explicitly use the SetNextItemOpen functions)
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
// Currently the main responsibility of this functions being to setup clip-rect + horizontal layout + menu navigation layer.
...
// To use it to need to call the functions SetTabItemClosed() between BeginTabBar() and EndTabBar().
