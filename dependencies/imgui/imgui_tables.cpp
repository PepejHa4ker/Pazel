...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        {
            // Hidden column: clear a few fields and we are done with it for the remainder of the functions.
...
void ImGui::TableBeginCell(ImGuiTable* table, int column_n)
{
    ...
    // To allow ImGuiListClipper to functions we propagate our row height
    if (!column->IsEnabled)
...
// This functions reorder draw channels based on matching clip rectangle, to facilitate merging them. Called by EndTable().
// For simplicity we call it TableMergeDrawChannels() but in fact it only reorder channels + overwrite ClipRect,
// actual merging is done by table->DrawSplitter.Merge() which is called right after TableMergeDrawChannels().
//
// Columns where the contents didn't stray off their local clip rectangle can be merged. To achieve
// this we merge their clip rect and make them contiguous in the channel list, so they can be merged
// by the call to DrawSplitter.Merge() following to the call to this functions.
...
// This functions assume that each column are pointing to a distinct draw channel,
...
//
// This functions is particularly tricky to understand.. take a breath.
